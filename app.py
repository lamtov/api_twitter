...
def api():
    if flask.request.method == 'POST':
        ...
        list_user_data=open(path_file_user_data, encoding="utf8").readlines()[:88]
        list_user_data=[x.replace('\n','') for x in list_user_data]
        list_proxy=open(path_file_proxy, encoding="utf8").readlines()[:10]
        list_proxy = [x.replace('\n', '') for x in list_proxy]
        list_twitter_bot_users = config.list_twitter_bot_users
        for i in range(0, number_threads):
            _list_user_data=list_user_data[i*int(len(list_user_data)/number_threads):(i+1)*int(len(list_user_data)/number_threads)]
            _list_proxy = list_proxy[i * int(len(list_proxy) / number_threads):(i + 1) * int(
                len(list_proxy) / number_threads)]
            _list_twitter_bot_users = list_twitter_bot_users[i * int(len(list_twitter_bot_users) / number_threads):(i + 1) * int(
                len(list_twitter_bot_users) / number_threads)]
            spam_thread=SpamThread(_list_twitter_bot_users, _list_user_data,_list_proxy, spam_message, path_file_picture)
